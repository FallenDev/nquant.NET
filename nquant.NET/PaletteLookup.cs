...
class PaletteLookup
{
    ...
    {
        var alphas = from pixel in palette select pixel.Alpha;
        var maxAlpha = alphas.Max();
        var uniqueAlphas = alphas.Distinct().Count();

        var reds = from pixel in palette select pixel.Red;
        var maxRed = reds.Max();
        var uniqueReds = reds.Distinct().Count();

        var greens = from pixel in palette select pixel.Green;
        var maxGreen = greens.Max();
        var uniqueGreens = greens.Distinct().Count();

        var blues = from pixel in palette select pixel.Blue;
